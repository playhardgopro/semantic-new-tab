...
async function createBookmarks() {
	...
	if (typeof faviconServerURL === 'undefined') {
		browser.storage.local.set({
			faviconServerURL: 'https://besticon-demo.herokuapp.com',
		});
		faviconServerURL = 'https://besticon-demo.herokuapp.com';
	}
	...
	let currentFolderBookmarks = allBookmarks.filter(
		({ type, parentId }) =>
			type === 'bookmark' &&
			parentId === (prefs.shownFolderId || 'toolbar_____')
	);
	currentFolderBookmarks = currentFolderBookmarks.map((el) => {
		...
		if (bookmark.url?.startsWith('http')) {
			...
			bookmark.faviconURL =
				'https://besticon-demo.herokuapp.com/icon?size=180&url=https://www.mozilla.org/ru/firefox/central/';
		return bookmark;
	});
...
export const MainContainer: React.FC = () => {
	...
	useEffect(() => {
		const loadBookmarks = async () => {
			...
			setBookmarks((prevState) => [...prevState, ...createdBookmarks]);
		};
		loadBookmarks();
		setCustomCss();
	}, []);
