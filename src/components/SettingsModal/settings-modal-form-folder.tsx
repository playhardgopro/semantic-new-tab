...
interface BookmarkFolders extends Bookmark {
	type: 'folder'
}
interface Option {
	key: number
...
export const SettingsModalFormFolder: React.FC<SettingsModalFormFolderProps> = ({
	bookmarksFolder,
}) => {
	...
	const updateSelect = async () => {
		const allItems = await browser.bookmarks.search({})
		const folders = getAll('folder', allItems) as BookmarkFolders[]
		const selectOptions = folders.map<Option>(({
			id, title,
		}, i) => (
			{
				key: i,
