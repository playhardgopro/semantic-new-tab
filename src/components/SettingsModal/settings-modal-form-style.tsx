...
async function updateStyle() {
	const customCss = (
		document.getElementById('reactCss') as HTMLTextAreaElement
	).value;
...
async function renderBookmarkFolder() {
	...
	const serverDomain = document.getElementById(
		'serverDomain'
	) as HTMLTextAreaElement;
	serverDomain.value =
		prefs.faviconServerURL || 'https://besticon-demo.herokuapp.com';
	const bookmarksFolder = (await browser.bookmarks.get(
		prefs.shownFolderId || 'toolbar_____'
	)) as Bookmark[];
	ReactDOM.render(
		<SettingsModalFormFolder bookmarksFolder={bookmarksFolder[0]} />,
		document.getElementById('bookmark-folder')
	);
}
...
export const SettingsModalFormStyle: React.FC = () => {
	...
	return (
		<Form id="styleForm">
			...
			<Label
				pointing="below"
				content="This CSS will be added as a style tag in the head section of the document."
			/>
			<Form.TextArea
				style={{
					minHeight: 300,
